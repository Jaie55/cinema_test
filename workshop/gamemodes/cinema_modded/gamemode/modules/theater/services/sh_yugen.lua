...
-- Nuevas funciones para cargar videos desde una URL específica
function SERVICE:GetURLInfo(url)
    if url.path then
        local entry, anime, episode = url.path:match("watch/(%d+)/([%a%d-_]+)/(%d+)")
        if entry and anime and episode then
            return { Data = ("%s,%s,%s"):format(entry, anime, episode) }
        end
    end
    return false
end
